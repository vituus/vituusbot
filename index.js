...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Vitor🧙‍♂️\n' 
            + 'ORG:Dono do EFX BOT;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5521987646723:+55 (21) 9876-46723\n' 
            + 'END:VCARD'
...
tiringa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nSeja bem vindo(a) ao projeto efx ${mdata.subject}`
    ...
    tiringa.on('CB:Blocklist', json => {
      ...
      tiringa.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = [`5521987646723@s.whatsapp.net`]
			const ownerNumberB = []
...
//_TIPO DE USUÁRIO
if (groupAdmins) {prema = 'Ademir🤓'}if (isPremium) {prema = 'Premium😎'} if (isOwner) {prema = 'El Vitor🕴🏽'} if (isUser) {prema = 'Registrado 🥳'} else {var prema = 'Membro comum🗿'}
...
if ((budy.match("Vitor")) || (budy.match("Vitor"))) {
tiringa.updatePresence(from, Presence.composing)
reply(Meu Criador maravilhoso instagram.com/Vitor_errejota`)
}
if (budy === "pnc") {
tiringa.updatePresence(from, Presence.composing)
reply(`vsfd ${pushname}`)
...
case 'Efx':
case 'EFX':
if (!isGroup) return reply( 'www.instagram.com/projeto_efx')
break
...
tiringa.sendMessage('5521987646723@s.whatsapp.net', options, text, {quoted: mek})
...
case 'pão':
if (isOwner) return reply('Oi Vitor Supremo')
else {
